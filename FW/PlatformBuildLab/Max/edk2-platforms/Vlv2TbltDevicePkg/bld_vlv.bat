...
echo CORE_PATH=%WORKSPACE%\edk2
echo PLATFORM_PATH=%WORKSPACE%\edk2-platforms
echo EDK_TOOLS_BIN=%WORKSPACE%\edk2-BaseTools-win32
echo PACKAGES_PATH=%WORKSPACE%\edk2-platforms;%WORKSPACE%\silicon;%WORKSPACE%\edk2
echo.
...
if /i "%~1"=="/p3" (
    set Build_Flags=%Build_Flags% -n 3
    set /a build_threads=3
    shift
    goto OptLoop
)
if /i "%~1"=="/p5" (
    set Build_Flags=%Build_Flags% -n 5
    set /a build_threads=5
    shift
    goto OptLoop
)
:: Required argument(s)
...
if /i "%~2" == "RELEASE" (
    set target=RELEASE
    set Build_Flags=%Build_Flags% -D SYMBOLIC_DEBUG=FALSE -D LOGGING=FALSE
    echo BUILD_TYPE = R >> Conf\BiosId.env
) else (
    set target=DEBUG
    set Build_Flags=%Build_Flags% -D SYMBOLIC_DEBUG=TRUE -D LOGGING=TRUE
...
if "%TOOL_CHAIN_TAG%" == "" (
  ...
  echo  --ERROR: VS2008/VS2010/VS2012/VS2013/VS2015 not installed correctly. 
...
echo TARGET.TXT is the following
type Conf\target.txt
echo.
::**********************************************************************
:: Build BIOS
...
echo Building with the Build_Flags = %Build_Flags%  
